  ...
  jqAjax(server + 'api/tags/', 'GET', {}, function (reply) {
    ...
    if (reply.code == 0) {
      ...
      $('.js-send-login').click(function () {
        ...
        jqAjax(server + "api/userLogin/", 'POST', JSON.stringify(params), function (reply) {
...
function getRecommendedTags() {
  ...
  chrome.tabs.query(queryOptions).then(function (tabs) {
    ...
    jqAjax(server + 'api/getKeyword/', 'POST', JSON.stringify({
      text: tab.title
    }), function (reply) {
...
function getBeforeTags() {
  ...
  chrome.tabs.query(queryOptions).then(function (tabs) {
    ...
    jqAjax(server + 'api/bookmarkByUrl/', 'GET', {
      url: tab.url
    }, function (reply) {
      if (reply.code != 0) {
        return;
...
(function (window) {
  ...
  chrome.storage.local.get({
    bookmarkServer: 'http://49.232.31.142:2000/'
  }, function (items) {
    ...
    chrome.tabs.query(queryOptions).then(function (tabs) {
      ...
      $('.js-send-bookmark').click(() => {
        ...
        if (!/http(s)?:\/\/.*/.test(params.url)) {
          ...
          console.log('send bookmark click');
